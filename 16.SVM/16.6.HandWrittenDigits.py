...
if __name__ == "__main__":
	...
	data = np.loadtxt(wkzutils.get_path_sources("16.optdigits.tra"), dtype=np.float, delimiter=',')
	x, y = np.split(data, (-1,), axis=1)
	images = x.reshape(-1, 8, 8)
	y = y.ravel().astype(np.int)

	print 'Load Test Data Start...'
	data = np.loadtxt(wkzutils.get_path_sources("16.optdigits.tes"), dtype=np.float, delimiter=',')
