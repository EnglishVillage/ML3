...
if __name__ == "__main__":
	data = np.loadtxt(wkzutils.get_path_sources("16.bipartition.txt"), dtype=np.float, delimiter='\t')
	x, y = np.split(data, (2,), axis=1)
	# xgboost需要做这个操作,svm不用
	# y[y == 0] = -1
	# 将100*1的二维数组 转变为100的一维数组
	y = y.ravel()

	# 分类器
	clf_param = (
		('linear', 0.1), ('linear', 0.5), ('linear', 1), ('linear', 2), ('rbf', 1, 0.1), ('rbf', 1, 1), ('rbf', 1, 10),
		('rbf', 1, 100), ('rbf', 5, 0.1), ('rbf', 5, 1), ('rbf', 5, 10), ('rbf', 5, 100))
	x1_min, x1_max = x[:, 0].min(), x[:, 0].max()  # 第0列的范围
	x2_min, x2_max = x[:, 1].min(), x[:, 1].max()  # 第1列的范围
	x1, x2 = np.mgrid[x1_min:x1_max:200j, x2_min:x2_max:200j]  # 生成网格采样点,x1和x2都是200*200二维数组
	...
	for i, param in enumerate(clf_param):
		...
		print '正确率：%.2f%%' % (accuracy_score(y_hat, y) * 100)
		# 画图
		print title
		# 2边支撑向量的分别个数
		...
		grid_hat = grid_hat.reshape(x1.shape)  # 使之与输入的形状相同,40000长度一维数组变为200*200二维数组
		#C代表着色方案
